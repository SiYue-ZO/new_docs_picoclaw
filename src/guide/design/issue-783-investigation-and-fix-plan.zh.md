...
- 现象：当 `agents.*.model.primary/fallbacks` 使用 `model_name` 别名（如 `step-3.5-flash`）时，fallback 链路将别名当作真�?`provider/model` 解析，导�?`provider` 可能为空、`model` 可能错误�?
- 根因：`ResolveCandidates` 仅对字符串做 `ParseModelRef`，未先通过 `model_list` 将别名映射到真实 `model` 字段�?
- 影响�?
  - fallback 执行可能把别名直接发�?OpenAI-compatible provider，触�?`Unknown Model`�?
...
- 补充测试：覆盖别名、嵌套路径模型（�?`openrouter/stepfun/...`）、空默认 provider�?
- 验证代码风格：与当前仓库风格保持一致（命名、错误处理、测试结构）�?

## 3. 联网最佳实践调研结论（已完成）

- [x] 查阅 OpenAI-compatible 网关（如 OpenRouter）对 `model` 字段的推荐处理�?
- [x] 查阅�?provider/fallback 设计最佳实践（候选解析、日志可观测性）�?
- [x] 将外部建议映射为本仓库可执行约束�?

外部参考要点（来自 OpenRouter/LiteLLM/Cloudflare AI Gateway 等官方文档）�?

- 优先显式配置，不依赖字符串切分推�?provider�?
- 对网关模型标识应保留完整路径语义，避免截断导�?Unknown Model�?
- fallback �?primary 应复用同一解析策略，避免“主路径正确、降级路径错误”�?
...
- �?fallback candidate 构建阶段先做 `model_name -> model_list.model` 映射�?
- 未命中映射时保留旧解析行为，保证兼容性�?
- 用新增测试锁定“别�?+ 嵌套模型路径 + 空默�?provider”场景�?

## 4. 实施步骤（顺序执行）

- [x] Step 1: 对齐现有代码模式，定位最小改动点（`pkg/agent` + `pkg/providers`）�?
- [x] Step 2: 实现“基�?model_list �?fallback 候选解析”�?
- [x] Step 3: 增加/更新单元测试，覆�?issue 场景�?
...
- 状态：已完�?
- 已完成改动：
  - `pkg/providers/fallback.go`：新�?`ResolveCandidatesWithLookup`，并保持 `ResolveCandidates` 向后兼容�?
  ...
  - `pkg/agent/instance_test.go`：新�?agent 侧别名解析到嵌套模型路径、无协议模型解析测试�?
- 风格对齐检查（完成）：�?`pkg/providers/fallback_test.go`、`pkg/providers/model_ref_test.go` 现有模式一致�?
