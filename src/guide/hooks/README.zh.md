...
1. 进程�?hook
2. 进程�?process hook（`JSON-RPC over stdio`�?
...
| 类型 | 接口 | 作用阶段 | 能否改写 |
| --- | --- | --- | --- |
| 观察�?| `EventObserver` | EventBus 广播事件�?| �?|
| LLM 拦截�?| `LLMInterceptor` | `before_llm` / `after_llm` | �?|
| Tool 拦截�?| `ToolInterceptor` | `before_tool` / `after_tool` | �?|
| Tool 审批�?| `ToolApprover` | `approve_tool` | 否，返回批准/拒绝 |
...
当前配置�?`hooks.defaults` 中统一设置�?

- `observer_timeout_ms`
- `interceptor_timeout_ms`
- `approval_timeout_ms`

注意：当前实现还没有单个 process hook 自己�?`timeout_ms` 字段，超时配置是全局默认值�?

## 快速开�?

如果你的目标只是先把当前 hook 流程跑通并观察到实际请求，最省事的是先用下面�?Python process hook 示例�?

1. 打开 `hooks.enabled`
2. 把下面文档里�?Python 示例保存到本地文件，例如 `/tmp/review_gate.py`
...
如果你是在开�?PicoClaw 本体，而不是只想验证协议，那么再看后面�?Go in-process 示例�?

## 两个示例的定�?

- Go in-process 示例
  适合验证宿主内的 hook 链路、理�?`MountHook()` 和各个同步点�?
- Python process 示例
  适合理解 `JSON-RPC over stdio` 协议、确认宿主和外部进程之间的消息来回是否正�?
...
你可以把它保存成你自己的 Go 文件，例�?`pkg/myhooks/example_logger.go`�?
...
如果你只需要代码挂载，直接�?`AgentLoop` 初始化后调用�?
...
当前 hook 系统支持 builtin hook，但这要求你自己�?factory 编进二进制。也就是说，下面这段注册代码需要和上面�?hook 定义一起放进你的工程里�?
...
- 如果设置�?`log_file`，它会把每次 hook 调用�?JSON Lines 写入文件
- 如果没有设置 `log_file`，它仍然会把摘要写到 gateway 日志
- 普通只�?LLM 的请求，通常会看�?`before_llm` �?`after_llm`
- 触发工具调用的请求，通常还会看到 `before_tool`、`approve_tool`、`after_tool`
- 如果 `log_events=true`，还会额外看�?`event`
...
如果你只看到�?`before_llm` / `after_llm`，没有看�?tool 相关阶段，通常不是 hook 没挂上，而是这次请求本身没有触发工具调用�?

## Python process hook 示例

下面这段脚本是一个最小的 `process hook` 示例。它只使�?Python 标准库，支持�?
...
你可以把它保存到任意本地路径，例�?`/tmp/review_gate.py`�?
...
- gateway 日志
  用来观察宿主是否成功启动了外部进程，以及脚本写到 `stderr` 的事件摘�?
- `PICOCLAW_HOOK_LOG_FILE`
  用来观察脚本实际收到了什么请求、返回了什么响�?

典型判断方式�?

- 只看�?`hook.hello`
  说明进程启动并完成握手了，但还没有新的业�?hook 请求真正打进�?
- 看到 `hook.event`
  说明 `observe` 配置生效�?
- 看到 `hook.before_tool`
  说明 `intercept: ["before_tool", ...]` 生效�?
...
- `id` 会随着当前进程内的请求递增；如�?hook 进程重启，计数会重新开�?
...
- PicoClaw 启动外部进程
- 请求和响应都按“一行一�?JSON 消息”传�?
- `hook.event` �?notification，不需要响�?
- `hook.before_llm` / `hook.after_llm` / `hook.before_tool` / `hook.after_tool` / `hook.approve_tool` �?request/response

当前宿主不会接受 process hook 主动发起的新 RPC。也就是说，外部 hook 现在只能“响�?PicoClaw 的调用”，不能反向调用宿主去发�?channel 消息�?
...
1. `hooks.enabled` 是否�?`true`
2. 对应�?builtin/process hook 是否 `enabled`
3. process hook �?`command` 路径是否正确
4. 你看的是否是正确的日志文�?
5. 当前请求是否真的走到了对应阶�?
6. `observe` / `intercept` 是否包含了你想看的点�?

一个很实用的最小排查组合是�?

- 先用文档里的 Python process 示例确认外部协议没问�?
- 再用文档里的 Go in-process 示例确认宿主内的 hook 链路没问�?

如果前者有 `hook.hello` 但没有业务请求，通常不是协议挂了，而是当前这次请求没有真正触发对应�?hook 点位�?
...
- 外部 hook 主动�?channel 消息
- 挂起 turn 并等待人工审批回�?
- inbound/outbound 全链路消息拦�?

如果你要做人审流转，推荐�?hook 作为审批入口，把审批状态机�?channel 交互放到独立�?`ApprovalManager`�?
