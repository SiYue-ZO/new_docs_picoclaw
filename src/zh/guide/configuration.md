...
旧版 `providers` 配置**已废弃**，但仍向后兼容。完整迁移指南见 [docs/migration/model-list-migration.md](/guide/migration/model-list-migration.md)。
...
| 主题 | 说明 |
| ---- | ---- |
| [Hook 系统](/guide/hooks/) | 事件驱动 Hook：观察者、拦截器、审批 Hook |
| [Steering](/guide/steering.md) | 在工具调用间向运行中的 Agent 注入消息 |
| [SubTurn](/guide/subturn.md) | 子 Agent 协调、并发控制、生命周期管理 |
| [上下文管理](/guide/agent-refactor/context.md) | 上下文边界检测、主动预算检查、压缩策略 |
